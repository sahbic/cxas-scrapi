...
def _render_turn(
  ...
  if wh := turn.get("webhook_call"):
    ...
    yield string.Template(call_item_tpl).substitute(
        ...
        RESPONSE=html.escape(json.dumps(wh.get("response") or {}, indent=2)),
    )
  if tl := turn.get("tool_call"):
    yield string.Template(call_item_tpl).substitute(
        ...
        RESPONSE=html.escape(json.dumps(tl.get("response") or {}, indent=2)),
    )
...
def assemble_report(
  ...
  floating_actions = component_loader("floating_actions/floating_actions.html")
  ...
  container_hydrated = main_container_template.substitute(CONTENT=full_content)
